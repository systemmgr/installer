...
_apimgr_completion() {
  ...
  CONFDIR="$HOME/.config/myscripts/apimgr"
  SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/apimgr}"
  #####################################################################
  SHOW_COMP_OPTS=""
  #####################################################################
  SHORTOPTS=""
  ...
  if [[ ${cur} == --no* ]]; then
    ...
    --yes-*)
      ...
      return 0
      ;;
    --repo|--user|--org|--token|--title|--body|--branch|--base|--tag|--provider)
      COMPREPLY=($(compgen -W '' -- "$cur"))
      return 0
      ;;
    --state)
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # enable completions
  complete -F _apimgr_completion -o default apimgr
