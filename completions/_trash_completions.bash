...
_trash() {
  ...
  list_all() { COMPREPLY=($(compgen -W '${LIST}' -- "$cur")); }
  local CASJAYSDEVDIR="${CASJAYSDEVDIR:-/usr/local/share/CasjaysDev/scripts}"
  local cur prev words cword opts split
  local cur="${COMP_WORDS[$COMP_CWORD]}"
  local prev="${COMP_WORDS[$COMP_CWORD - 1]}"
  local CONFFILE="settings.conf"
  local CONFDIR="$HOME/.config/myscripts/trash"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/trash}"
  ...
  local trashlist="$(\find -L "$trashdir"/* -type f,d 2>/dev/null)"
  local LIST="${trashlist//$trashdir\//}"
  local ARRAY="list restore remove empty"
