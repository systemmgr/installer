...
_mybrowser() {
  ...
  local CONFDIR="$HOME/.config/myscripts/mybrowser"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/mybrowser}"
  ...
  if [[ ${cur} == --* ]]; then
    ...
    *)
      ...
      if [[ "$ARRAY" = "show__none" ]]; then
        ...
        COMPREPLY=($(compgen -W '${ARRAY}' -o default -- "${cur}"))
        compopt -o nospace
      elif [ -n "$OPTS" ]; then
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # enable completions
  complete -F _mybrowser mybrowser
