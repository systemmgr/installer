...
_systemmgr() {
  ...
  local CONFDIR="$HOME/.config/myscripts/systemmgr"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/systemmgr}"
  ...
  if [[ ${cur} == --* ]]; then
    ...
    update | cron | remove | version)
      local prev="$special"
      k=0
      i="/usr/local/share/CasjaysDev/apps/systemmgr"
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # enable completions
  complete -F _systemmgr systemmgr
