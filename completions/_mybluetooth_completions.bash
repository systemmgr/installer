...
_mybluetooth() {
  ...
  _bluetooth_addresses() { [[ -n ${COMP_BLUETOOTH_SCAN:-} ]] && COMPREPLY+=($(compgen -W "$(hcitool scan | awk '/^\t/{print $1}')" -- "$cur")); }
  _bluetooth_devices() { COMPREPLY+=($(compgen -W "$(hcitool dev | awk '/^\t/{print $1}')" -- "$cur")); }
  local CASJAYSDEVDIR="${CASJAYSDEVDIR:-/usr/local/share/CasjaysDev/scripts}"
  local cur prev words cword opts split
  local cur="${COMP_WORDS[$COMP_CWORD]}"
  local prev="${COMP_WORDS[$COMP_CWORD - 1]}"
  local CONFFILE="settings.conf"
  local CONFDIR="$HOME/.config/myscripts/mybluetooth"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/mybluetooth}"
  ...
  local SHORTOPTS=""
  local ARRAY="config status connect disconnect pair unpair search discover"
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # enable completions
  complete -F _mybluetooth mybluetooth
