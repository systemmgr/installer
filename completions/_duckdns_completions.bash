...
# @Created           :  Thursday, Dec 09, 2021 13:28 EST
# @File              :  duckdns
...
_duckdns() {
  ...
  local CONFDIR="$HOME/.config/myscripts/duckdns"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/duckdns}"
  ...
  if [[ ${cur} == --* ]]; then
    ...
    ssl)
      local prev="--version"
      COMPREPLY=($(compgen -W 'renew cron create' -- "${cur}"))
      ;;
    cron)
      local prev="--version"
      COMPREPLY=($(compgen -W 'install remove run' -- "${cur}"))
      ;;
    *)
