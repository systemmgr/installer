...
_gitadmin() {
  ...
  local ARRAY="project is show shorten web latest newsite stow s status l log init setup reset lfs diff branch backup commit update modify create "
  local ARRAY+="delete push pull clone merge merge-resolve rebase switch squash remote list gist single user org github gitlab private issues tag release "
  local ARRAY+="bitbucket codeberg forgejo gitea gogs azure aws cron updater "
