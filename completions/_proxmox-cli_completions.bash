  ...
  if [[ ${cur} == --* ]]; then
    ...
    list | download | delete | info)
      case "${COMP_WORDS[1]}" in
        iso)
          case "$prev" in
            list | info) COMPREPLY=() ;;  # No more arguments
            download) ;; # Expects URL - no completion
            delete) ;; # Expects volid - no completion  
          esac
          ;;
      esac
      ;;
    # LXC subcommands  
    templates | create | start | stop | console)
      case "${COMP_WORDS[1]}" in
        lxc)
          case "$prev" in
            templates) COMPREPLY=() ;;  # No more arguments
            create) ;; # Expects vmid and template - no completion
            start | stop | status | console) ;; # Expects vmid - no completion
          esac
          ;;
      esac
      ;;
    ...
    clone | snapshot)
      case "${COMP_WORDS[1]}" in
        vm)
          case "$prev" in
            clone | snapshot) ;; # Expects vmid - no completion
          esac
          ;;
      esac
      ;;
