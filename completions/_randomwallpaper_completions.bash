...
_randomwallpaper() {
  ...
  local CONFDIR="$HOME/.config/myscripts/randomwallpaper"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/randomwallpaper}"
  ...
  if [[ ${cur} == --* ]]; then
    ...
    bg)
      COMPREPLY=($(compgen -W '$BGOPTS' -- "$cur"))
      return
      ;;
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # enable completions
  complete -F _randomwallpaper randomwallpaper
