...
_web-notify_completion() {
  ...
  CONFDIR="$HOME/.config/myscripts/web-notify"
  SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/web-notify}"
  #####################################################################
  SHOW_COMP_OPTS=""
  #####################################################################
  SHORTOPTS=""
  SHORTOPTS+=""
  #####################################################################
  LONGOPTS="--completions --config --debug --dir --help --options --no-color --version "
  LONGOPTS+="--silent --force --discord-token --slack-token --telegram-token --telegram-channel"
  #####################################################################
  ARRAY="discord slack telegram"
  ARRAY+=""
  #####################################################################
  LIST=""
  LIST+=""
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # enable completions
  complete -F _web-notify_completion -o default web-notify
