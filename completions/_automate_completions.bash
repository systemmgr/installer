...
_automate_completion() {
  ...
  local CONFDIR="$HOME/.config/myscripts/automate"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/automate}"
  ...
  local SHORTOPTS=""
  local ARRAY="play stop pause toggle state"
  ...
  if [[ ${cur} == --* ]]; then
    ...
    state)
      prev="state"
      [[ $cword -le 2 ]] && COMPREPLY=($(compgen -W 'json text' -- "$cur")) ||
        COMPREPLY=($(compgen -W '${ARRAY}' -- "$cur"))
      ;;
    *)
      COMPREPLY=($(compgen -W '${ARRAY}' -- "$cur"))
