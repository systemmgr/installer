...
_pkmgr() {
    ...
    if [[ -f "$cache_file" ]] && [[ $(($(date +%s) - $(stat -c %Y "$cache_file" 2>/dev/null || stat -f %m "$cache_file" 2>/dev/null || echo 0))) -lt $cache_age ]]; then
      ...
      Linux)
        if command -v apt-cache &>/dev/null; then
          ...
          # Include AUR packages if --enable-aur is set and an AUR helper exists
          if [[ "$enable_aur" == "true" ]] && command -v paru &>/dev/null; then
            paru -Slq 2>/dev/null | head -1000 | tee "$cache_file"
          elif [[ "$enable_aur" == "true" ]] && command -v yay &>/dev/null; then
            yay -Slq 2>/dev/null | head -1000 | tee "$cache_file"
          else
  ...
  local LONGOPTS+="--force --enable-aur --no-color --enable-log"
  ...
  # Check if --enable-aur is in the command line
  local enable_aur=false
  local cmd_index=1
  for word in "${COMP_WORDS[@]:1}"; do
    if [[ "$word" == "--enable-aur" ]]; then
      enable_aur=true
      ((cmd_index++))
    elif [[ "$word" != -* ]]; then
      break
  ...
  if [[ ${cur} == --* ]]; then
    ...
    case "${COMP_WORDS[$cmd_index]:-$prev}" in
    --options)
      local prev="--options"
    ...
    --enable-aur)
      local prev="--enable-aur"
      # After --enable-aur, show available commands
