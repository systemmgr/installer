...
_tasks() {
  ...
  local CONFDIR="$HOME/.config/myscripts/tasks"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/tasks}"
  ...
  if [[ ${cur} == --* ]]; then
    ...
    info | done | session | switch | stop | start | delete)
