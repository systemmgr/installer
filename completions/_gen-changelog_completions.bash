...
_gen-changelog_completion() {
  ...
  CONFDIR="$HOME/.config/myscripts/gen-changelog"
  SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/gen-changelog}"
  #####################################################################
  SHOW_COMP_OPTS=""
  #####################################################################
  SHORTOPTS=""
  ...
  LONGOPTS+="--author --contributors --pr --issue --link-commits --github-url "
  LONGOPTS+="--undo --categorize --conventional --silent"
  #####################################################################
  ARRAY="init"
  ARRAY+=""
  #####################################################################
  ...
  if [[ ${cur} == --* ]]; then
    ...
    --dir)
      ...
      ;;
    --format)
      COMPREPLY=($(compgen -W 'markdown json yaml html keepachangelog' -- "$cur"))
      ;;
    --files | --changelog | --message)
      _filedir
      ;;
    *)
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # enable completions
  complete -F _gen-changelog_completion -o default gen-changelog
