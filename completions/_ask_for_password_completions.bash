...
# - - - - - - - - - - - - - - - - - - - - - - - - -
_ask_for_password_completion() {
  ___findcmd() { find -L "${1:-$CONFDIR/}" -maxdepth ${3:-3} -type ${2:-f} 2>/dev/null | sed 's#'${1:-$CONFDIR}'##g' | grep '^' || return 1; }
  local CONFDIR="$HOME/.config/myscripts/ask_for_password"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/ask_for_password}"
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # enable completions
  complete -F _ask_for_password_completion ask_for_password
