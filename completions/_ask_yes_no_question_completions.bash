...
# - - - - - - - - - - - - - - - - - - - - - - - - -
_ask_yes_no_question_completion() {
  ___findcmd() { find -L "${1:-$CONFDIR/}" -maxdepth ${3:-3} -type ${2:-f} 2>/dev/null | sed 's#'${1:-$CONFDIR}'##g' | grep '^' || return 1; }
  local CONFDIR="$HOME/.config/myscripts/ask_yes_no_question"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/ask_yes_no_question}"
  ...
  if [[ ${cur} == --* ]]; then
    ...
    --debug | --no-color | --help | --version | --config | --options)
      ...
      ;;
    *)
      [[ $cword -le 1 ]] && COMPREPLY=($(compgen -W '${ARRAY}' -- "$cur")) ||
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # enable completions
  complete -F _ask_yes_no_question_completion ask_yes_no_question
