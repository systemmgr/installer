...
_devhints() {
  ...
  local CONFDIR="$HOME/.config/myscripts/devhints"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/devhints}"
  #local SEARCHCMD="$(___findcmd "$SEARCHDIR/" "d" "1" | sort -u)"
  local SHOW_COMP_OPTS=""
  local FILEDIR=""
  local OPTS=""
  local LONGOPTS="--completions --debug --no-color --options --config --version --help --silent --dir --refresh"
  local SHORTOPTS=""
  local ARRAY="list search "
