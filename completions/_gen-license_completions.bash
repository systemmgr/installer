...
_gen-license() {
  ...
  local CONFDIR="$HOME/.config/myscripts/gen-license"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/gen-license}"
  local SEARCHCMD="$(___findcmd $CONFDIR/files/ f 2 && ___findcmd $CONFDIR/custom/ f 2)"
  local SHOW_COMP_OPTS=""
  local FILEDIR=""
  local OPTS="-list -config -copy -download"
  local LONGOPTS="--completions --debug --no-color --options --config --version --help --silent --dir"
  local SHORTOPTS=""
  local ARRAY="$(echo "$SEARCHCMD" | sort -u | sed 's/\.[^.]*$//')"
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # enable completions
  complete -F _gen-license gen-license
