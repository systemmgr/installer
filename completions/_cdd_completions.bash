...
# @@Changelog        :  Migrated to self-contained; __-prefix fixes; UUOC fix
...
__cdd_completion() {
  ...
  #####################################################################
  __cdd_ls() { ls -A "$1" 2>/dev/null | grep -v -- '^$' | grep -- '^' || false; }
  __cdd_grep() { GREP_COLORS="" grep -shE -- '^.*=*..*$' "$1" 2>/dev/null | sed 's|"||g' 2>/dev/null | grep -- '^' || false; }
  __cdd_findcmd() { find -L "${1:-$CONFDIR/}" -maxdepth ${3:-3} -type ${2:-f} 2>/dev/null | sed 's#'${1:-$CONFDIR}'##g' | grep -- '^' || return 1; }
  #####################################################################
  ...
  #####################################################################
  ARRAY="$(__cdd_ls "${CDD_OPTIONS_PROJECT_DIR:-$HOME/.local/share/cdd/projects}")"
  ARRAY="$(echo "${ARRAY[@]}" && [ -f "$HOME/.config/myscripts/cdd/aliases.conf" ] && awk -F '=' '{print $1}' "$HOME/.config/myscripts/cdd/aliases.conf" | grep -v -- '#')"
  #####################################################################
  LIST=""
  LIST+=""
  #####################################################################
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # enable completions
  complete -F __cdd_completion -o default cdd
