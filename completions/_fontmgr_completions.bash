...
_fontmgr() {
  ...
  local CONFDIR="$HOME/.config/myscripts/fontmgr"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/fontmgr}"
  ...
  if [[ ${cur} == --* ]]; then
    ...
    update | cron | remove | version)
      prev="$special"
      k=0
      i="$HOME/.local/share/CasjaysDev/apps/fontmgr"
