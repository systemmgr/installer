...
_notifications() {
  ...
  local CONFDIR="$HOME/.config/myscripts/notifications"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/notifications}"
  #local SEARCHCMD="$(___findcmd "$SEARCHDIR/" "d" "1" | sort -u)"
  local SHOW_COMP_OPTS=""
  local FILEDIR=""
  local OPTS=""
  local LONGOPTS="--completions --debug --no-color --options --config --version --help --silent --dir --help --bg --all "
  local SHORTOPTS=""
  local ARRAY="config client"
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # enable completions
  complete -F _notifications notifications
