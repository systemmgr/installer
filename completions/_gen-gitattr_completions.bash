...
_gen-gitattr_completion() {
  ...
  CONFDIR="$HOME/.config/myscripts/gen-gitattr"
  SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/gen-gitattr}"
  #####################################################################
  SHOW_COMP_OPTS=""
  #####################################################################
  SHORTOPTS=""
  SHORTOPTS+=""
  #####################################################################
  LONGOPTS="--completions --config --debug --dir --help --options --no-color --version --silent --force --no- "
  LONGOPTS+=""
  #####################################################################
  #NOOPTS="--no-* "
  #####################################################################
  ...
  if [[ ${cur} == --* ]]; then
    ...
    --dir)
      ...
      ;;
    create | update | list | default)
      COMPREPLY=($(compgen -W '' -- "$cur"))
      ;;
    save | print)
      COMPREPLY=($(compgen -W '$(___find_rel "$CONFDIR/templates")' -- "$cur"))
      ;;
    *)
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # enable completions
  complete -F _gen-gitattr_completion gen-gitattr
