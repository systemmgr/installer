...
# @Description       :  Utility to keep multiple clipboards
# @TODO              :
# @Other             :
...
_multi_clipboard() {
  ...
  local CONFDIR="$HOME/.config/myscripts/multi_clipboard"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/multi_clipboard}"
  ...
  local SHORTOPTS="-z -s -s -I -o -O -g -W -w -x -x -S"
  local ARRAY=""
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # enable completions
  complete -F _multi_clipboard multi_clipboard
