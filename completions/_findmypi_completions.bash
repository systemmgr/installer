...
_findmypi() {
  ...
  local CONFDIR="$HOME/.config/myscripts/findmypi"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/findmypi}"
  ...
  if [[ ${cur} == --* ]]; then
    ...
    *)
      if [[ ${cword} == 2 ]]; then
        COMPREPLY=($(compgen -W '{0..9}' -- "${cwords}"))
        compopt -o nospace
      else
        prev=ip
        COMPREPLY=($(compgen -W '${ARRAY} {0..9} .' -- "${cur}"))
        compopt -o nospace
      fi
