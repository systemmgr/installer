...
_desktopmgr() {
  ...
  local CONFDIR="$HOME/.config/myscripts/desktopmgr"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/desktopmgr}"
  ...
  if [[ ${cur} == --* ]]; then
    ...
    update | cron | remove | version)
      prev="$special"
      k=0
      i="$HOME/.local/share/CasjaysDev/apps/desktopmgr"
    ...
    qemu)
      prev="qemu"
      COMPREPLY=($(compgen -W 'linux windows macos arm' -- "${cur}"))
      ;;
    *)
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # enable completions
  complete -F _desktopmgr desktopmgr
