...
_tor-site() {
  ...
  local CONFDIR="$HOME/.config/myscripts/tor-site"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/tor-site}"
  ...
  local SHORTOPTS=""
  local ARRAY="status stop service create"
  ...
  if [[ ${cur} == --* ]]; then
    ...
    service)
      local prev="service"
      COMPREPLY=($(compgen -W 'create install remove run' -- "${cur}"))
      ;;
