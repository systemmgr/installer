...
_config() {
  ...
  ___git() { git --git-dir="$CONFIG_CWD_BARE_DIR" --work-tree="$CONFIG_CWD_HOME_DIR" "$@" 2>/dev/null; }
  local CASJAYSDEVDIR="${CASJAYSDEVDIR:-/usr/local/share/CasjaysDev/scripts}"
  local cur prev words cword opts split
  local cur="${COMP_WORDS[$COMP_CWORD]}"
  local prev="${COMP_WORDS[$COMP_CWORD - 1]}"
  local CONFFILE="settings.conf"
  local CONFDIR="$HOME/.config/myscripts/config"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/config}"
  local CONFIG_SYSTEM_HOME="${CONFIG_SYSTEM_HOME:-$HOME/.config/myscripts/config/home}"
  local CONFIG_GIT_BARE_DIR="${CONFIG_GIT_BARE_DIR:-$HOME/.local/share/myscripts/config/bare}"
  local CONFIG_SYSTEM_BACKUP_DIR="${CONFIG_SYSTEM_BACKUP_DIR:-$HOME/.local/share/myscripts/config/backups}"
  ...
  if [[ ${cur} == --* ]]; then
    ...
    commit)
      local prev="config"
      COMPREPLY=($(compgen -W '$(___git commit)' -- "$cur"))
      return
      ;;

    push)
      local prev="push"
      COMPREPLY=($(compgen -W '$(___git push)' -- "$cur"))
      return
      ;;

    pull)
      local prev="pull"
      COMPREPLY=($(compgen -W '$(___git pull)' -- "$cur"))
      return
      ;;

    set)
      local prev="set"
      COMPREPLY=($(compgen -W '$(___git config --local)' -- "$cur"))
      return
      ;;
    ...
    clone)
      ...
      [[ ${cword} -gt 3 ]] && return
      [[ ${cword} -eq 3 ]] && _filedir
      COMPREPLY=($(compgen -W 'http:// https:// git:// ssh://' -- "$cur"))
      return
      ;;
...
complete -F ___git_complete _config __git_main
