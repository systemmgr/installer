...
# - - - - - - - - - - - - - - - - - - - - - - - - -'
_tldr() {
  ___findcmd() { find -L ${1:-$CONFDIR/} -maxdepth ${3:-3} -type ${2:-f} -printf '%f\n' 2>/dev/null | sed 's#'${1:-$CONFDIR}'##g;s|\.md||g' | grep '^' || return 1; }
  local CASJAYSDEVDIR="${CASJAYSDEVDIR:-/usr/local/share/CasjaysDev/scripts}"
  local cur prev words cword opts split
  local cur="${COMP_WORDS[$COMP_CWORD]}"
  local prev="${COMP_WORDS[$COMP_CWORD - 1]}"
  local cachedir="$HOME/.cache/tldr"
  local CONFFILE="settings.conf"
  local CONFDIR="$HOME/.config/myscripts/tldr"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/tldr}"
  local SEARCHCMD="$(___findcmd "$cachedir/" "f,l" "5")"
  ...
  COMPREPLY=($(compgen -W '$(___tldr)' -- "${cur}"))
  if [[ ${cur} == --* ]]; then
