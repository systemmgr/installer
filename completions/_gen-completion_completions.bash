...
# @@Created          :  Tuesday, Jul 19, 2022 00:24 EDT
# @@File             :  gen-completion
# @@Description      :
# @@Changelog        :
...
# - - - - - - - - - - - - - - - - - - - - - - - - -
_gen-completion_completion() {
  ___findcmd() { find -L "${1:-$CONFDIR/}" -maxdepth ${3:-3} -type ${2:-f} 2>/dev/null | sed 's#'${1:-$CONFDIR}'##g' | grep '^' || return 1; }
  local CONFDIR="$HOME/.config/myscripts/gen-completion"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/gen-completion}"
  ...
  local SHORTOPTS=""
  local ARRAY="format help array list long short search case"
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # enable completions
  complete -F _gen-completion_completion gen-completion
