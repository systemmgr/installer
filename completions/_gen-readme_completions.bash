...
_gen-readme() {
  ...
  local CONFDIR="$HOME/.config/myscripts/gen-readme"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/gen-readme}/files"
  local SEARCHCMD="$(___findcmd "$SEARCHDIR/" "d" "1" | sort -u | sed 's/\.[^.]*$//')"
  ...
  if [[ ${cur} == --* ]]; then
    ...
    all)
      COMPREPLY=($(compgen -W '$(___findcmd "$SEARCHDIR/" "f" "4")' -- "$cur"))
      return
      ;;

    projects)
      COMPREPLY=($(compgen -W '$(___findcmd "$SEARCHDIR/projects/" "f" "1")' -- "$cur"))
      return
      ;;

    installers)
      COMPREPLY=($(compgen -W '$(___findcmd "$SEARCHDIR/installers/" "f" "1")' -- "$cur"))
      return
      ;;
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # enable completions
  complete -F _gen-readme gen-readme
