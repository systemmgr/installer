...
_thememgr() {
  ...
  local CONFDIR="$HOME/.config/myscripts/thememgr"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/thememgr}"
  ...
  if [[ ${cur} == --* ]]; then
    ...
    update | cron | remove | version)
      prev="$special"
      k=0
      i="$HOME/.local/share/CasjaysDev/apps/thememgr"
