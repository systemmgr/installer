...
# @File              :  dockermgr
# @Description       :  Comprehensive Docker container management and orchestration tool
# @TODO              :
# @Other             :
# @Resource          :
# - - - - - - - - - - - - - - - - - - - - - - - - -'
_dockermgr() {
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  local LIST="airsonic ampache apache2 aria2 buildah bun caddy cherokee coder community coolify couchdb daapd ddns deno dictd docker dockerproxy "
  local LIST+="enclosed flaresolverr gitea gohttpserver hello-world hub ifconfig it-tools jekyll jenkins joplin lenpaste lidarr lighttpd mariadb "
  local LIST+="mongodb navidrome nextcloud nginx nginx-manager oidc opengist php pihole podman portainer postgres prowlarr proxy-manager radarr "
  local LIST+="rancher rarbg readarr readmes redis registry search soft-serve sonarr stikked stirlingpdf supabase tor tor-browser traefik "
  local LIST+="transmission tududi uptime vault watchtower webvirtmgr wireguard yacht ympd youtubedl-web "
  ...
  if [[ ${cur} == --* ]]; then
    ...
    list | status | helm | available | active | inactive | init | rancher | k3b | ds | multiarch | swarm | compose | proxy)
      local prev="endopts"
      COMPREPLY=($(compgen -W '' -- "${cur}"))
      ;;
    ...
    test | enter | exec | connect | run | log | restart | attach)
      local prev="test"
