...
_calc_completion() {
  ...
  local CONFDIR="$HOME/.config/myscripts/calc"
  local SEARCHDIR="${CONFDIR:-$HOME/.config/myscripts/calc}"
  ...
  local FILEDIR=""
  local LONGOPTS="--completions --debug --no-color --options --config --version --help --silent --term --dir "
  local SHORTOPTS=""
  local ARRAY="$(echo {0..9}) '*' + - '/'"
