...
#if [ ! -t 0 ] && { [ "$1" = --term ] || [ $# -eq 0 ]; }; then { [ "$1" = --term ] && shift 1 || true; } && TERMINAL_APP="TRUE" myterminal -e "${0##*/} $*" && exit || exit 1; fi
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Set script title
...
[ -n "${CASJAYSDEV_TITLE_PREV+x}" ] || [ -n "${CASJAYSDEV_TITLE_SET+x}" ] || {
  if [ -t 1 ]; then
    ...
    IFS= read -rs -d $'\033' -t 0.5 _t </dev/tty 2>/dev/null || true
    _t="${_t#]l}"
    [ -n "$_t" ] && CASJAYSDEV_TITLE_PREV="$_t"
    unset _t
  fi
}
...
GEN_SCRIPT_REPLACE_ENV_OUTPUT_COLOR_2="${GEN_SCRIPT_REPLACE_ENV_OUTPUT_COLOR_2:-6}"
...
# - - - - - - - - - - - - - - - - - - - - - - - - -
# script functions
# __printf_color "text" "$PRINTF_SET_COLOR"
...
__printf_head() { __printf_color "$1" "$PRINTF_SET_PURPLE"; }
__printf_opts() { __printf_color "$1" "$PRINTF_SET_CYAN"; }
__printf_line() { __printf_color "$1" "$PRINTF_SET_BLUE"; }
# - - - - - - - - - - - - - - - - - - - - - - - - -
# list all packages
...
__trap_exit_GEN_SCRIPT_REPLACE_FILENAME() {
  ...
  unset CASJAYSDEV_TITLE_SET && printf '\033]2;%s\033\\' "${CASJAYSDEV_TITLE_PREV:-$SHELL}"
...
# - - - - - - - - - - - - - - - - - - - - - - - - -
__notify_remote() {
  local cmd="${GEN_SCRIPT_REPLACE_ENV_REMOTE_NOTIFY_COMMAND%% *}"
...
[ -d "$GEN_SCRIPT_REPLACE_ENV_CLONE_DIR" ] || mkdir -p "$GEN_SCRIPT_REPLACE_ENV_CLONE_DIR" &>/dev/null
...
while :; do
  ...
  --options)
    ...
    [ -n "$1" ] || __printf_color "Current options for ${PROG:-$APPNAME}" "$PRINTF_SET_BLUE"
